	...
	switch ( filterKey ) {
		...
		case 'bounces':
			return 'include' === value ?
				__( 'Bounced', 'burst-mainwp' ) :
				__( 'Active', 'burst-mainwp' );

		case 'new_visitor':
			return 'include' === value ?
				__( 'New', 'burst-mainwp' ) :
				__( 'Returning', 'burst-mainwp' );

		case 'entry_exit_pages':
			if ( 'entry' === value ) {
				return __( 'Entry', 'burst-mainwp' );
			}
			if ( 'exit' === value ) {
				return __( 'Exit', 'burst-mainwp' );
			}
			return value;

		case 'status':
			if ( '200' === value ) {
				return __( 'OK', 'burst-mainwp' );
			}
			if ( '404' === value ) {
				return __( 'Not Found', 'burst-mainwp' );
			}
			if ( 'all' === value ) {
				return __( 'All statuses', 'burst-mainwp' );
			}
			return value;
