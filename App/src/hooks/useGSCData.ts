...
const useGSCData = (): UseGSCDataReturn => {
	...
	const connect = useCallback( () => {
		...
		const finish = ( connected: boolean, reason: string | null = null ) => {
			...
			if ( connected ) {
				...
				toast.success( __( 'Google Search Console connected.', 'burst-mainwp' ) );
				return;
			}
			void queryClient.invalidateQueries({ queryKey: [ 'gscStatus' ] });
			if ( reason ) {
		...
		doAction( 'gsc_connect', {}).then( ( res: any ) => { // eslint-disable-line @typescript-eslint/no-explicit-any
			if ( 'locked' === res?.error ) {
				...
				popupRef.current = null;
				setIsConnecting( false );
				setError( __( 'Another administrator is currently connecting. Please try again shortly.', 'burst-mainwp' ) );
				return;
			}
			if ( ! res?.url || ! popup ) {
				popup?.close();
				popupRef.current = null;
				setIsConnecting( false );
				setError( __( 'Could not start the connection. Please try again.', 'burst-mainwp' ) );
				return;
			}
			...
			const poll = async() => {
				if ( Date.now() - startedAt > POLL_TIMEOUT ) {
					finish( false, __( 'The connection timed out. Please try again.', 'burst-mainwp' ) );
					return;
				}
				...
				if ( result?.connect_failed ) {
					...
					finish( false, __( 'The connection was not completed. Please try again.', 'burst-mainwp' ) );
					return;
				}
	...
	const { mutate: disconnectMutate, isPending: isDisconnecting } = useMutation({
		...
		onSuccess: () => {
			...
			toast.success( __( 'Google Search Console disconnected.', 'burst-mainwp' ) );
		}
	});
