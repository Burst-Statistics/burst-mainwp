...
const addABTestIcon = ( content, row ) => {
	...
	if ( 'no_winner' === row.significant ) {
		...
		tooltip = __( 'Winner of the A/B test with a probability of >95%.', 'burst-mainwp' );
		color = 'gold';
		name = 'trophy';
		styleClass = 'text-primary font-semibold';
	} else {
		tooltip = __( 'Least performant version of the A/B test with a probability of >95%.', 'burst-mainwp' );
		color = 'gray';
		name = 'frown';
		styleClass = 'text-text-gray';
	}
