...
const SOURCE_DESCRIPTIONS = {
	search: __( 'Visitors who found you through a search engine like Google, Bing or DuckDuckGo. No ad spend involved, just organic results.', 'burst-mainwp' ),
	social: __( 'Traffic from social networks like Facebook, Instagram, LinkedIn or Reddit, either from posts, profiles or link shorteners like t.co.', 'burst-mainwp' ),
	referral: __( 'Someone clicked a link to your site from another website. Not a search engine, not social, just a regular link somewhere on the web.', 'burst-mainwp' ),
	aiReferral: __( 'Visitors who clicked a link in an AI tool like ChatGPT, Perplexity or Claude. A new channel worth watching as AI-generated answers increasingly link to sources.', 'burst-mainwp' ),
	paid: __( 'Traffic from ads. Detected via click IDs like gclid (Google Ads) or msclkid (Bing Ads), or a UTM medium tagged as cpc, ppc or paid.', 'burst-mainwp' ),
	email: __( 'Visitors from an email campaign or newsletter. Relies mostly on UTM parameters since most email clients strip the referrer before the visit reaches your site.', 'burst-mainwp' ),
	direct: __( 'No referrer, no UTM parameters, no click IDs. Most analytics tools call this "Direct", which implies someone typed your URL by hand. In reality, this bucket also catches clicks from desktop apps, messaging tools, PDFs, browser extensions and any visit where tracking context got stripped along the way.', 'burst-mainwp' )
};
...
function SourcesBarTooltip({ data, interval }) {
	...
	const metricLabel = METRIC_POPOVER_OPTIONS[ selectedMetric ]?.label ?? __( 'Visitors', 'burst-mainwp' );
			...
			<table className="w-full border-collapse text-sm">
				<thead>
					<tr className="border-b border-gray-200 text-xs text-gray-600">
						<th className="pb-1.5 text-left font-medium">{ __( 'Source', 'burst-mainwp' ) }</th>
				...
				<tfoot>
					<tr className="border-t border-gray-200">
						<td className="pt-1.5 text-gray-700">{ __( 'Total', 'burst-mainwp' ) }</td>
...
const INTERVAL_OPTIONS = [
	{ value: 'auto', label: __( 'Auto', 'burst-mainwp' ) },
	{ value: 'day', label: __( 'Day', 'burst-mainwp' ) },
	{ value: 'week', label: __( 'Week', 'burst-mainwp' ) },
	{ value: 'month', label: __( 'Month', 'burst-mainwp' ) }
];

const METRIC_POPOVER_OPTIONS = {
	visitors: { label: __( 'Visitors', 'burst-mainwp' ), default: true },
	pageviews: { label: __( 'Pageviews', 'burst-mainwp' ) },
	sessions: { label: __( 'Sessions', 'burst-mainwp' ) }
};
	...
	const renderIntervalSelector = ( pendingValue, setPendingValue ) => (
		<div className="flex flex-col gap-1.5">
			<span className="text-xs font-semibold text-text-gray uppercase tracking-wide">
				{ __( 'Group by', 'burst-mainwp' ) }
			</span>
			<div
				role="radiogroup"
				aria-label={ __( 'Group by', 'burst-mainwp' ) }
				...
				{ INTERVAL_OPTIONS.map( ( option ) => {
					...
					return (
						<button
							...
							title={
								isDisabled ?
									__( 'Too many data points for this date range', 'burst-mainwp' ) :
									undefined
							}
...
const SourcesChartBlock = ( props ) => {
	...
	return (
		<Block className="row-span-2 @lg:col-span-12 @xl:col-span-9 group/root">
			<BlockHeading
				title={ __( 'Sources over time', 'burst-mainwp' ) }
				isReport={ isReport }
					...
					<div className="flex items-center gap-2">
						...
						<Popover.Root>
							<Popover.Trigger asChild>
								<button
									type="button"
									aria-label={ __( 'Source definitions', 'burst-mainwp' ) }
							...
							>
								<div className="border-b border-gray-100 px-4 py-3">
									<h5 className="m-0 text-sm font-semibold text-text-black">
										{ __( 'Source definitions', 'burst-mainwp' ) }
									</h5>
								</div>
