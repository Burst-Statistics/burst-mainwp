...
const InCompleteDataNotice = memo( () => {
    ...
    const title = isCity ?
        sprintf(
            __( 'Region-level data is available for visits after %s.', 'burst-mainwp' ),
            formatUnixToDate( availableTime )
        ) :
        sprintf(
            __( 'Country-level data is available for visits after %s.', 'burst-mainwp' ),
            formatUnixToDate( availableTime )
        );

    const description = isCity ?
        __( 'Region tracking is a new feature, so this data is only available for visits recorded after it was enabled.', 'burst-mainwp' ) :
        __( 'Country tracking is a new feature, so this data is only available for visits recorded after it was enabled.', 'burst-mainwp' );
