		...
		<Block className={className}>
			<BlockHeading
				...
				title={<>
					<MetricInfo metricKey="reading_engagement" side="bottom">
						{__( 'Reading engagement', 'burst-mainwp' )}
					</MetricInfo>
					{/* Expand to overlay. */}
					<button
						type="button"
					...
					<div className="flex items-center gap-2">
						...
						<label className="flex cursor-pointer items-center gap-1.5 text-xs text-text-gray select-none">
							<Checkbox.Root
								...
								aria-label={__( 'Show lowest reading engagement pages', 'burst-mainwp' )}
							...
							{__( 'Lowest engagement', 'burst-mainwp' )}
						</label>
					</div>
				}
			/>
			<BlockContent className="px-0 py-0 overflow-y-auto">
				<BarDataTable
					...
					isLoading={isLoading}
					emptyState={
						__( 'No reading engagement data recorded yet.', 'burst-mainwp' )
					}
				/>
			</BlockContent>
		</Block>
