		...
		<Block className={className}>
			<BlockHeading
				...
				title={<>
					<MetricInfo metricKey="search_terms" side="bottom">
						{__( 'Website searches', 'burst-mainwp' )}
					</MetricInfo>
					{/* Expand to overlay. */}
					<button
						type="button"
					...
					<div className="flex items-center gap-2">
						...
						<label className="flex cursor-pointer items-center gap-1.5 text-xs text-text-gray select-none">
							<Checkbox.Root
								...
								aria-label={__( 'Show only searches with no results', 'burst-mainwp' )}
							...
							{__( 'Without results', 'burst-mainwp' )}
						</label>
					</div>
				}
			/>
			<BlockContent className="px-0 py-0 overflow-y-auto">
				<BarDataTable
					...
					emptyState={
						noResultsOnly ?
							__( 'No zero-result searches found.', 'burst-mainwp' ) :
							__( 'No search terms recorded yet.', 'burst-mainwp' )
					}
				/>
			</BlockContent>
		</Block>
