...
const ConnectedSearchConsoleBlock = ({ propertyStatus }: ConnectedSearchConsoleBlockProps ): JSX.Element => {
	...
	const message = 'none' === propertyStatus ?
		__( 'No Search Console property contains this site. Add a URL-prefix property in Search Console, then Burst will retry automatically.', 'burst-mainwp' ) :
		__( 'Checking your Google Search Console properties…', 'burst-mainwp' );

	return (
		<Block className='row-span-2 overflow-hidden @xl:col-span-6'>
			<BlockHeading title={ __( 'Google Searches', 'burst-mainwp' ) } />
...
const SearchConsoleBlock = (): JSX.Element => {
	...
	return (
		<OverlayBlock
			title={ __( 'Google searches', 'burst-mainwp' ) }
			blurLabel={ __( 'Google searches', 'burst-mainwp' ) }
