	...
	const options = [
		{
			...
			label: __( 'Previous period', 'burst-mainwp' )
		},
		{
			value: COMPARE_MODES.YEAR_OVER_YEAR,
			label: __( 'Year over year', 'burst-mainwp' )
		}
	];

	return (
		<div className='flex flex-col justify-start py-2 px-4'>
			<h3 className='font-medium text-md pt-4 pb-1'>{__( 'Compare mode', 'burst-mainwp' )}</h3>
			<p className='text-sm text-text-gray pb-2'>{__( 'Some blocks show a comparison of data. Select how these blocks compare the data.', 'burst-mainwp' )}</p>
