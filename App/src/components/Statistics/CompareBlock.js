...
const CompareBlock = ( props ) => {
	...
	if ( includePageMetrics ) {
		metrics.reading_engagement_score = __( 'Engagement score', 'burst-mainwp' );
	}
	metrics.bounce_rate = __( 'Bounce Rate', 'burst-mainwp' );
	if ( includePageMetrics ) {
		metrics.conversions = __( 'Conversions', 'burst-mainwp' );
	} else {
		metrics.avg_time_on_page = __( 'Avg. time on page', 'burst-mainwp' );
	}
	const emptyData = {};
			...
			{ Object.keys( data ).map( ( key, i ) => {
				...
				if ( showCommunityComparison && m.communityMetricKey ) {
					...
					if ( ! anonymousUsageDataEnabled ) {
						communityTooltipText = __( 'Opt in to data sharing to see how your site compares to peers.', 'burst-mainwp' );
						...
						if ( metricConfig.showAverage && null != communityMetric?.average && Number.isFinite( Number( communityMetric.average ) ) ) {
							communityTooltipText = sprintf(
								__( 'Websites with similar traffic average %s%% new visitors.', 'burst-mainwp' ),
							...
							if ( null !== betterThanPercent ) {
								communityTooltipText = sprintf(
									__( 'Your site performs better than %d%% of websites with similar traffic.', 'burst-mainwp' ),
									betterThanPercent
								);
							}
						}
