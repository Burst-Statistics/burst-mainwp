...
const DataTableBlock = ( /** @type {BlockComponentProps} */ props ) => {
	...
	const config = {
		...
		search_terms: {
			label: __( 'Search terms', 'burst-mainwp' ),
			searchable: true,
			defaultColumns: [ 'term', 'volume', 'results' ],
			columnsOptions: {
				term: {
					label: __( 'Search term', 'burst-mainwp' ),
					default: true,
					format: 'string',
					align: 'left',
					group_by: true
				...
				volume: {
					label: __( 'Volume', 'burst-mainwp' ),
					format: 'integer',
					align: 'right'
				},
				results: {
					label: __( 'Results', 'burst-mainwp' ),
		...
		outgoing_links: {
			...
			columnsOptions: {
				...
				previous_clicks: {
					label: __( 'Prev. clicks', 'burst-mainwp' ),
					category: 'traffic',
					pro: true,
					align: 'right'
				},
				previous_clicks_yoy: {
					label: __( 'Prev. clicks YoY', 'burst-mainwp' ),
					category: 'traffic',
					pro: true,
					align: 'right'
				}
		...
		forms: {
			...
			columnsOptions: {
				...
				form_provider_label: {
					label: __( 'Provider', 'burst-mainwp' ),
					default: false,
					format: 'string',
					align: 'left',
					group_by: true
				...
				submissions: {
					label: __( 'Submissions', 'burst-mainwp' ),
					category: 'traffic',
					align: 'right'
				},
				pageviews: {
				...
				previous_submissions: {
					label: __( 'Prev. submissions', 'burst-mainwp' ),
					category: 'traffic',
					pro: true,
					align: 'right'
				},
				previous_pageviews: {
					label: __( 'Prev. visitors', 'burst-mainwp' ),
					category: 'traffic',
					pro: true,
					align: 'right'
				},
				previous_conversion_rate: {
					label: __( 'Prev. conv. rate', 'burst-mainwp' ),
					category: 'engagement',
					format: 'percentage',
					pro: true,
					align: 'right'
					...
					const badge = 0 < count ? (
						<span className="shrink-0 rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-xs font-medium text-blue-700">
							{sprintf(
								...
								_n( '%d parameter', '%d parameters', count, 'burst-mainwp' ),
								count
							)}
						</span>
					) : null;
				...
				<label className="flex cursor-pointer items-start gap-2.5 py-1">
					...
					<span className="flex flex-col gap-0.5">
						<span className="text-sm font-medium text-text-black">
							{__( 'Show parameters', 'burst-mainwp' )}
						</span>
						...
						<>
							{ ! isInOverlay && ! isReport && (
								<button
									...
									aria-label={ __( 'Expand table', 'burst-mainwp' ) }
									title={ __( 'Expand table', 'burst-mainwp' ) }
								>
			...
			{ 0 < totalRows && (
				<BlockFooter className="border-t border-gray-200 gap-4">
					<div className="flex items-center gap-2 text-sm text-gray-600">
						<span>{ __( 'Rows per page:', 'burst-mainwp' ) }</span>
						<select
					...
					<div className="flex items-center gap-1">
						<span className="mr-2 text-sm text-gray-600">
							{ `${ ( currentPage - 1 ) * rowsPerPage + 1 }-${ Math.min( currentPage * rowsPerPage, totalRows ) } ${ __( 'of', 'burst-mainwp' ) } ${ totalRows }` }
						</span>
						<button
							type="button"
							...
							aria-label={ __( 'First page', 'burst-mainwp' ) }
						>
						...
						<button
							...
							disabled={ 1 === currentPage }
							aria-label={ __( 'Previous page', 'burst-mainwp' ) }
						>
						...
						<button
							...
							aria-label={ __( 'Next page', 'burst-mainwp' ) }
						>
						...
						<button
							...
							disabled={ currentPage === totalPages }
							aria-label={ __( 'Last page', 'burst-mainwp' ) }
						>
