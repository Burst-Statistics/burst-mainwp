...
const DEFAULT_TITLE = __( 'New chat', 'burst-mainwp' );
const LOADING_STEPS = [
	__( 'Understanding your question...', 'burst-mainwp' ),
	__( 'Fetching analytics data...', 'burst-mainwp' ),
	__( 'Preparing a concise answer...', 'burst-mainwp' )
];
...
const ChatAssistantModal = ({ isOpen, onClose }: ChatAssistantModalProps ) => {
	...
	const suggestions = useMemo( () => [
		{
			text: __( 'How many pageviews did we get today?', 'burst-mainwp' ),
			icon: 'pageviews'
		},
		{
			text: __( 'What are the top traffic sources?', 'burst-mainwp' ),
			icon: 'referrers'
		},
		{
			text: __( 'Are there any active visitors on my site right now?', 'burst-mainwp' ),
			icon: 'visitors'
		},
		{
			text: __( 'Which pages are the most popular this month?', 'burst-mainwp' ),
			icon: 'file'
		}
	], []);
	...
	useEffect( () => {
		...
		if ( isOpen && 'chat_assistant_prompt_select' === currentStepId && ! hasInitializedTourSessionRef.current ) {
			...
			setSessions( ( prev ) => {
				...
				const userQuestion = __( 'How many pageviews did we get today?', 'burst-mainwp' );
			...
			>
				...
				>
					...
					<button
						...
						title={
							isSidebarOpen ?
								__( 'Collapse sidebar', 'burst-mainwp' ) :
								__( 'Expand sidebar', 'burst-mainwp' )
						}
					>
						<Icon
				...
				{! isSidebarOpen && (
					<div className="flex h-[48px] items-center justify-center">
						<Tooltip content={__( 'New chat', 'burst-mainwp' )}>
							<button
								type="button"
								onClick={createNewChat}
								className="rounded-md p-1.5 text-text-gray transition-all duration-150 ease-in-out hover:bg-gray-100 hover:text-text-black hover:shadow-sm"
						...
						<div className="flex flex-col py-2">
							<div className="rounded-xl border border-dashed border-gray-300 bg-gray-50/50 p-5 text-center">
								<h3 className="text-sm font-semibold text-text-black mb-1">
									{__( 'Welcome to Burst AI Assistant!', 'burst-mainwp' )}
								</h3>
												...
												<span className="truncate pr-1">
													<Select.Value>
														{selectedModel ? (
															...
															defaultModelLabel ? `${__( 'Default model', 'burst-mainwp' )} (${defaultModelLabel})` : __( 'Default model', 'burst-mainwp' )
														)}
													</Select.Value>
												</span>
	...
	return (
		<>
			<Modal
				isOpen={isOpen}
				onClose={onClose}
				title={__( 'Burst AI chat', 'burst-mainwp' )}
				subtitle={__(
			...
			<Modal
				...
				title={__( 'Delete chat', 'burst-mainwp' )}
