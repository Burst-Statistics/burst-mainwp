...
const FILTER_OPERATOR_TOOLTIP_DESCRIPTIONS = {
	is: __( 'Showing data where this value matches.', 'burst-mainwp' ),
	'is-not': __( 'Hiding data where this value matches.', 'burst-mainwp' ),
	'is-any-of': __( 'Showing data that matches any of these values.', 'burst-mainwp' ),
	'is-not-any-of': __( 'Hiding data that matches any of these values.', 'burst-mainwp' )
};
...
const FilterChip = ({
	...
	const getValueLabel = () => {
		if ( isTwoValues ) {
			return `${ safeDecodeURI( filter.values[0].display ) } ${ __( 'or', 'burst-mainwp' ) } ${ safeDecodeURI( filter.values[1].display ) }`;
		}
