...
const ImagePickerControl =
	...
	({ field, fieldState, label, help, className, isDarkPreview = false, darkToggle = null, ...props }) => {
		...
		const mediaTitle = props.setting?.media_title ?? __( 'Select an image', 'burst-mainwp' );
		const mediaButton = props.setting?.media_button ?? __( 'Set image', 'burst-mainwp' );
...
const DarkModeImagePicker = ({ field, control, ...props }) => {
	...
	const toggle = (
		<Switch.Root
			...
			disabled={props.disabled}
			aria-label={__( 'Toggle dark mode image', 'burst-mainwp' )}
			title={
				showDark ?
					__( 'Switch to light mode image', 'burst-mainwp' ) :
					__( 'Switch to dark mode image', 'burst-mainwp' )
			}
