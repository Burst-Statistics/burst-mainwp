...
const getTemplateDefaultHtml = () => {
	const footerText = sprintf(
		...
		__( 'This e-mail is sent from your own WordPress website, which is: %s.<br />If you don\'t want to receive these e-mails in your inbox, please go to the Burst settings page on your website and remove your email from the recipients in the report settings or contact the administrator of your website.', 'burst-mainwp' ),
		'{domain}'
	);

	return `
	<h2>${ __( 'Find out more', 'burst-mainwp' ) }</h2>
		<p>${ __( 'Dive deeper into your analytics and uncover new opportunities for {domain}.', 'burst-mainwp' ) }</p>
		<div style="margin-top: 24px;">
			<a href="#" class="burst-button" style="display: inline-block; border-radius: 4px; padding: 16px 24px; font-size: 16px; font-weight: 600; line-height: 1; color: #ffffff; text-decoration: none;">
				${ __( 'Explore insights', 'burst-mainwp' ) }
			</a>
		</div>
...
const readThemeColors = () => {
	const scope = document.getElementById( 'burst-mainwp' ) || document.body;
					...
					setup: ( editor ) => {
						...
						if ( 'email_footer' === setting?.id ) {
							...
							editor.addButton( 'templates', {
								type: 'menubutton',
								icon: 'template',
								tooltip: __( 'Templates', 'burst-mainwp' ),
								menu: [
									{
										text: __( 'Template 1 (default)', 'burst-mainwp' ),
										onclick: () => applyTemplate( getTemplateDefaultHtml() )
									},
									{
										text: __( 'Template 2', 'burst-mainwp' ),
						...
						if ( ! isDisabled ) {
							editor.addButton( 'html_source', {
								title: __( 'Source Code', 'burst-mainwp' ),
								text: '<>',
								onclick: () => {
							...
							content={
								<div className="flex flex-col gap-1.5 text-xs">
									<p className="font-medium mb-0.5">
										{ __( 'Allowed HTML elements', 'burst-mainwp' ) }
									</p>
									{ ( emailMode ?
