		...
		<div className="w-full p-6">
				...
				<div className="flex items-center gap-3">
					...
					<div>
						<p className="font-semibold text-text-black">
							{ __( 'Google Search Console', 'burst-mainwp' ) }
						</p>
						<p className="text-sm text-text-gray">
							{ isConnected && __( 'Connected', 'burst-mainwp' ) }
							{ needsReconnect && __( 'Reconnection required', 'burst-mainwp' ) }
							{ 'disconnected' === status && __( 'Not connected', 'burst-mainwp' ) }
						</p>
					</div>
				</div>
				...
				<div className="flex items-center gap-2">
					...
					{ ! isConnected && (
						<ButtonInput btnVariant="primary" onClick={ () => setConnectOpen( true ) }>
							{ needsReconnect ? __( 'Reconnect', 'burst-mainwp' ) : __( 'Connect', 'burst-mainwp' ) }
						</ButtonInput>
					) }
					{ isConnected && (
						<ButtonInput
			...
			<p className="mt-4 text-sm text-text-gray">
				{ createInterpolateElement(
					__( 'Burst shows your Search Console data using read-only access. Your authorization is stored only on this site and the data is fetched directly from Google – nothing is stored on Burst’s servers. <a>Read our Search Console privacy policy</a>.', 'burst-mainwp' ),
					{
						a: (
							<a
			...
			{ isConnected && 'none' === propertyStatus && (
				<p className="mt-4 text-sm text-red">
					{ sprintf(
						...
						__( 'No matching property found — add a URL-prefix property for %s in Search Console.', 'burst-mainwp' ),
						siteUrl
					) }
				</p>
			) }
			...
			{ isConnected && 'paused' === propertyStatus && (
				<p className="mt-4 text-sm text-text-gray">
					{ sprintf(
						...
						__( 'Search Console access was temporarily denied. Stored data is retained and syncing will retry after %s.', 'burst-mainwp' ),
			...
			<Modal
				title={ __( 'Connect Google Search Console', 'burst-mainwp' ) }
				content={
					<div className="flex flex-col gap-4">
						<p className="text-text-gray">
							{ __( 'Burst will open a Google window where you can grant read-only access to your Search Console data. Keep this tab open while you authorize.', 'burst-mainwp' ) }
						</p>
						{ isConnecting && (
							<div className="flex items-center gap-2 text-text-gray">
								<Icon name="loading" size={ 20 } />
								<span>{ __( 'Waiting for authorization in the Google window…', 'burst-mainwp' ) }</span>
							</div>
						) }
						{ error && ! isConnecting && (
					...
					<>
						...
						<ButtonInput btnVariant="primary" onClick={ connect } disabled={ isConnecting }>
							<span className="inline-flex items-center gap-2">
								{ __( 'Connect with Google', 'burst-mainwp' ) }
							</span>
						</ButtonInput>
					</>
				}
			...
			<Modal
				title={ __( 'Disconnect Google Search Console?', 'burst-mainwp' ) }
				content={
					<p>
						{ __( 'This revokes Burst’s access to your Search Console data. You can reconnect at any time.', 'burst-mainwp' ) }
					</p>
				}
