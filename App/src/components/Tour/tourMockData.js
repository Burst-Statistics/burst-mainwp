...
const mockGoalFields = [
	...
	},
	{
		id: 'type',
		type: 'radio-buttons',
		label: __( 'What type of goal do you want to set?', 'burst-mainwp' ),
		options: {
			clicks: {
				label: __( 'Clicks', 'burst-mainwp' ),
				description: __( 'Track clicks on element', 'burst-mainwp' ),
				type: 'clicks',
				icon: 'mouse'
			},
			views: {
				label: __( 'Views', 'burst-mainwp' ),
				description: __( 'Track views of element', 'burst-mainwp' ),
				type: 'views',
				icon: 'eye'
			},
			visits: {
				label: __( 'Visits', 'burst-mainwp' ),
				description: __( 'Track visits to page', 'burst-mainwp' ),
				type: 'visits',
				icon: 'visitors'
			},
			hook: {
				label: __( 'Hook', 'burst-mainwp' ),
				description: __( 'Track execution of a WordPress hook', 'burst-mainwp' ),
				type: 'hook',
				icon: 'hook',
				server_side: true
			}
	...
	},
	{
		id: 'page_or_website',
		type: 'radio-buttons',
		label: __( 'Do you want to track a specific page or the entire website?', 'burst-mainwp' ),
		options: {
			page: {
				label: __( 'Page', 'burst-mainwp' ),
				description: __( 'Track page specific', 'burst-mainwp' ),
				type: 'page',
				icon: 'page'
			},
			website: {
				label: __( 'Website', 'burst-mainwp' ),
				description: __( 'Track on whole site', 'burst-mainwp' ),
				type: 'website',
				icon: 'website'
			}
		},
	...
	},
	{
		id: 'selector',
		type: 'selector',
		label: __( 'What element do you want to track?', 'burst-mainwp' ),
		disabled: false,
	...
	},
	{
		id: 'hook',
		type: 'hook',
		label: __( 'What hook do you want to track?', 'burst-mainwp' ),
		disabled: false,
		default: '',
		react_conditions: {
			type: [ 'hook' ]
	...
	},
	{
		...
		type: 'radio-buttons',
		label: __( 'What metric do you want to use to calculate the conversion rate?', 'burst-mainwp' ),
		options: {
			visitors: {
...
export const getFrontendTourMockData = ( path ) => {
		...
		return {
			...
			data: {
				reports: [
					{
						id: 1,
						name: __( 'Weekly Traffic & Top Pages Story', 'burst-mainwp' ),
						format: 'story',
						frequency: 'weekly',
						dayOfWeek: 'monday',
						sendTime: '09:00',
	...
	if ( 'get_action/get_filter_options' === cleanPath || 'get_filter_options' === cleanPath ) {
		...
		const mockFilterOptions = {
			...
			devices: [
				{ ID: 'desktop', name: __( 'Desktop', 'burst-mainwp' ), key: 'desktop' },
				{ ID: 'mobile', name: __( 'Mobile', 'burst-mainwp' ), key: 'mobile' },
				{ ID: 'tablet', name: __( 'Tablet', 'burst-mainwp' ), key: 'tablet' }
			],
			browsers: [
	...
	if ( 'data/insights' === cleanPath || 'insights' === cleanPath ) {
		...
		const metricConfig = {
			pageviews: { label: __( 'Pageviews', 'burst-mainwp' ), color: '#2A5B8C', base: 160, var1: 30, var2: 5 },
			visitors: { label: __( 'Visitors', 'burst-mainwp' ), color: '#F59E0B', base: 110, var1: 20, var2: 4 },
			sessions: { label: __( 'Sessions', 'burst-mainwp' ), color: '#10B981', base: 130, var1: 25, var2: 3 },
			bounces: { label: __( 'Bounces', 'burst-mainwp' ), color: '#EF4444', base: 45, var1: 12, var2: 2 },
			conversions: { label: __( 'Conversions', 'burst-mainwp' ), color: '#8B5CF6', base: 22, var1: 8, var2: 1 },
			bounce_rate: { label: __( 'Bounce rate', 'burst-mainwp' ), color: '#EC4899', base: 34, var1: 5, var2: 1 },
			avg_time_on_page: { label: __( 'Avg. time on page', 'burst-mainwp' ), color: '#6366F1', base: 85, var1: 15, var2: 2 }
		};
	...
	if ( 'data/today' === cleanPath || 'today' === cleanPath ) {
		return {
			...
			data: {
				...
				pageviews: { title: __( 'Total pageviews', 'burst-mainwp' ), value: '385' },
				timeOnPage: { title: __( 'Average time on page', 'burst-mainwp' ), value: '154' }
			}
		};
	}
	...
	if (
		...
		return {
			...
			data: {
				columns: [
					{ id: 'url', name: __( 'URL', 'burst-mainwp' ), format: 'url', sortable: true, align: 'left' },
					{ id: 'clicks', name: __( 'Clicks', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'previous_clicks', name: __( 'Previous', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' }
				],
				data: [
	...
	if (
		...
		return {
			...
			data: {
				columns: [
					{ id: 'form_title', name: __( 'Form', 'burst-mainwp' ), format: 'text', sortable: true, align: 'left' },
					{ id: 'submissions', name: __( 'Submissions', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'pageviews', name: __( 'Pageviews', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'conversion_rate', name: __( 'Conversion rate', 'burst-mainwp' ), format: 'percentage', sortable: true, align: 'right' }
				],
				data: [
					{
						form_id: '1',
	...
	if (
		...
		return {
			...
			data: {
				columns: [
					{ id: 'url', name: __( 'Page URL', 'burst-mainwp' ), format: 'url', sortable: true, align: 'left' },
					{ id: 'clicks', name: __( 'Clicks', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'previous_clicks', name: __( 'Previous', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' }
				],
				data: [
	...
	if (
		...
		if ( cleanPath.includes( 'campaigns' ) || 'campaigns' === query.get( 'type' ) || 'campaigns' === query.get( 'config' ) ) {
			return {
				...
				data: {
					columns: [
						{ id: 'campaign', name: __( 'Campaign', 'burst-mainwp' ), format: 'text', sortable: true, align: 'left' },
						{ id: 'pageviews', name: __( 'Pageviews', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
						{ id: 'visitors', name: __( 'Visitors', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
						{ id: 'conversions', name: __( 'Conversions', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
						{ id: 'conversion_rate', name: __( 'Conversion rate', 'burst-mainwp' ), format: 'percentage', sortable: true, align: 'right' }
					],
					data: [
						{
		...
		if ( cleanPath.includes( 'search_console' ) || 'search_console' === query.get( 'type' ) || 'search_console' === query.get( 'config' ) ) {
			return {
				...
				data: {
					columns: [
						{ id: 'query', name: __( 'Query', 'burst-mainwp' ), format: 'text', sortable: true, align: 'left' },
						{ id: 'clicks', name: __( 'Clicks', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
						{ id: 'impressions', name: __( 'Impressions', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
						{ id: 'ctr', name: __( 'CTR', 'burst-mainwp' ), format: 'percentage', sortable: true, align: 'right' },
						{ id: 'position', name: __( 'Position', 'burst-mainwp' ), format: 'number', sortable: true, align: 'right' }
					],
					data: [
		...
		if ( cleanPath.includes( 'countries' ) || 'countries' === query.get( 'type' ) || 'countries' === query.get( 'config' ) ) {
			return {
				...
				data: {
					columns: [
						{ id: 'country_code', name: __( 'Country', 'burst-mainwp' ), format: 'country', sortable: true, align: 'left' },
						{ id: 'country', name: __( 'Country', 'burst-mainwp' ), format: 'text', sortable: true, align: 'left' },
						{ id: 'visitors', name: __( 'Visitors', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
						{ id: 'bounce_rate', name: __( 'Bounce rate', 'burst-mainwp' ), format: 'percentage', sortable: true, align: 'right' }
					],
					data: [
		...
		if ( cleanPath.includes( 'referrers' ) || 'referrers' === query.get( 'type' ) || 'referrers' === query.get( 'config' ) ) {
			return {
				...
				data: {
					columns: [
						{ id: 'referrer', name: __( 'Referrer', 'burst-mainwp' ), format: 'text', sortable: true, align: 'left' },
						{ id: 'visitors', name: __( 'Visitors', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
						{ id: 'bounce_rate', name: __( 'Bounce rate', 'burst-mainwp' ), format: 'percentage', sortable: true, align: 'right' }
					],
					data: [
		...
		return {
			...
			data: {
				columns: [
					{ id: 'page_url', name: __( 'Page', 'burst-mainwp' ), format: 'url', sortable: true, align: 'left' },
					{ id: 'pageviews', name: __( 'Pageviews', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'visitors', name: __( 'Visitors', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'sessions', name: __( 'Sessions', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'bounce_rate', name: __( 'Bounce rate', 'burst-mainwp' ), format: 'percentage', sortable: true, align: 'right' },
					{ id: 'avg_time', name: __( 'Avg. time on page', 'burst-mainwp' ), format: 'time', sortable: true, align: 'right' },
					{ id: 'avg_time_on_page', name: __( 'Avg. time on page', 'burst-mainwp' ), format: 'time', sortable: true, align: 'right' },
					{ id: 'conversions', name: __( 'Conversions', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' },
					{ id: 'conversion_rate', name: __( 'Goal conv. rate', 'burst-mainwp' ), format: 'percentage', sortable: true, align: 'right' },
					{ id: 'first_time_visitors', name: __( 'First time visitors', 'burst-mainwp' ), format: 'integer', sortable: true, align: 'right' }
				],
				data: [
					{
	...
	if ( 'ecommerce/top-performers' === cleanPath ) {
		return {
			...
			data: {
				'top-product': {
					label: __( 'Top product', 'burst-mainwp' ),
			...
			data: [
				{
					...
					title: __( 'Recover Abandoned Carts', 'burst-mainwp' ),
					impact: 'high',
					description: __( '22% of carts were abandoned this week. Setting up a recovery email could recover ~$1,800/mo.', 'burst-mainwp' )
				},
				{
					id: 'mobile-checkout-speed',
					title: __( 'Optimize Mobile Checkout', 'burst-mainwp' ),
					impact: 'medium',
					description: __( 'Mobile conversion rate is 1.8% vs 4.2% on desktop. Streamlining checkout fields can boost sales.', 'burst-mainwp' )
				}
			]
		};
	}
	...
	if ( 'ecommerce/sales-funnel' === cleanPath ) {
		return {
			request_success: true,
			data: [
				{ id: 'view_product', stage: __( 'Product Views', 'burst-mainwp' ), label: __( 'Product Views', 'burst-mainwp' ), value: 3200, count: 3200, percentage: 100 },
				{ id: 'add_to_cart', stage: __( 'Add to Cart', 'burst-mainwp' ), label: __( 'Add to Cart', 'burst-mainwp' ), value: 680, count: 680, percentage: 21.25 },
				{ id: 'checkout', stage: __( 'Checkout Initiated', 'burst-mainwp' ), label: __( 'Checkout Initiated', 'burst-mainwp' ), value: 310, count: 310, percentage: 9.68 },
				{ id: 'purchase', stage: __( 'Completed Purchase', 'burst-mainwp' ), label: __( 'Completed Purchase', 'burst-mainwp' ), value: 182, count: 182, percentage: 5.68 }
			]
		};
	}
	...
	if ( 'ecommerce/subscriptions-retention' === cleanPath ) {
		return {
			...
			data: {
				...
				products: [
					{ id: 'all', name: __( 'All Products', 'burst-mainwp' ) },
	...
	if ( 'data/page-parameters' === cleanPath || 'page-parameters' === cleanPath ) {
		return {
			...
			data: {
				columns: [
					{ id: 'parameter', name: __( 'Parameter', 'burst-mainwp' ), align: 'left' },
					{ id: 'pageviews', name: __( 'Pageviews', 'burst-mainwp' ), align: 'right' },
					{ id: 'visitors', name: __( 'Visitors', 'burst-mainwp' ), align: 'right' }
				],
				data: [
	...
	if ( 'data/source-referrers' === cleanPath || 'source-referrers' === cleanPath ) {
		return {
			...
			data: {
				columns: [
					{ id: 'referrer', name: __( 'Referrer URL', 'burst-mainwp' ), align: 'left' },
					{ id: 'visitors', name: __( 'Visitors', 'burst-mainwp' ), align: 'right' }
				],
				data: [
