...
function getActionLabel( anchor: string, action: TourStepAction ): { label: string; icon: string } | null {
	...
	if ( anchor.includes( 'sheet-overlay-close' ) ) {
		return { label: __( 'Close', 'burst-mainwp' ), icon: '✖️' };
	}
	if ( anchor.includes( 'insights-metric-selector' ) || 'change_metrics' === action ) {
		return { label: __( 'Select metrics', 'burst-mainwp' ), icon: '📈' };
	}
	if ( anchor.includes( 'field-privacy_level' ) ) {
		return { label: __( 'Pick privacy level', 'burst-mainwp' ), icon: '🔒' };
	}
	...
	if ( anchor.includes( 'page-filter' ) || 'apply_filter' === action ) {
		return { label: __( 'Add filter', 'burst-mainwp' ), icon: '🔍' };
	}
	if ( anchor.includes( 'data-table-click-filter' ) ) {
		return { label: __( 'Click to filter', 'burst-mainwp' ), icon: '🔍' };
	}
	if ( anchor.includes( 'data-table-columns' ) || 'toggle_columns' === action ) {
		return { label: __( 'Toggle columns', 'burst-mainwp' ), icon: '⚙️' };
	}
	if ( anchor.includes( 'data-table-expand' ) ) {
		return { label: __( 'Expand table', 'burst-mainwp' ), icon: '📑' };
	}
	if ( anchor.includes( 'share-link-button' ) || 'generate_share_link' === action ) {
		return { label: __( 'Generate share link', 'burst-mainwp' ), icon: '🔗' };
	}
	if ( anchor.includes( 'community-comparison' ) ) {
		return { label: __( 'Hover me', 'burst-mainwp' ), icon: '🌐' };
	}
	if ( anchor.includes( 'date-range-all-time' ) ) {
		return { label: __( 'Click "All time"', 'burst-mainwp' ), icon: '📅' };
	}
	if ( anchor.includes( 'date-range' ) || 'change_date_range' === action ) {
		return { label: __( 'Open date range', 'burst-mainwp' ), icon: '📅' };
	}
	if ( anchor.includes( 'chat-prompt-suggestions' ) || anchor.includes( 'chat-prompt-item' ) ) {
		return { label: __( 'Click any prompt', 'burst-mainwp' ), icon: '💬' };
	}
	if ( anchor.includes( 'chat-assistant' ) ) {
		return { label: __( 'Open AI chat', 'burst-mainwp' ), icon: '🤖' };
	}
	if ( anchor.includes( 'subnav-goals' ) ) {
		return { label: __( 'Click Goals', 'burst-mainwp' ), icon: '🎯' };
	}
	if ( anchor.includes( 'subnav-general' ) ) {
		return { label: __( 'Click General', 'burst-mainwp' ), icon: '⚙️' };
	}
	if ( anchor.includes( 'subnav-features' ) ) {
		return { label: __( 'Click Features', 'burst-mainwp' ), icon: '🕒' };
	}
	if ( anchor.includes( 'subnav-advanced' ) ) {
		return { label: __( 'Click Advanced', 'burst-mainwp' ), icon: '👻' };
	}
	if ( anchor.includes( 'subnav-customization' ) ) {
		return { label: __( 'Click Customization', 'burst-mainwp' ), icon: '🎨' };
	}
	if ( anchor.includes( 'subnav-reports' ) ) {
		return { label: __( 'Click Reports', 'burst-mainwp' ), icon: '📊' };
	}
	if ( anchor.includes( 'new-report-button' ) ) {
		return { label: __( 'Click new report', 'burst-mainwp' ), icon: '➕' };
	}
	return { label: __( 'Click here', 'burst-mainwp' ), icon: '👆' };
}
