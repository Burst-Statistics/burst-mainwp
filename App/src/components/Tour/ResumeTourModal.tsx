...
const ResumeTourModal: React.FC<ResumeTourModalProps> = ({
		...
		<div className="flex items-center gap-3">
			...
			<div>
				<h3 className="text-lg font-bold leading-snug text-slate-900 dark:text-slate-100">
					{ __( 'Welcome back to the tour! 👋', 'burst-mainwp' ) }
				</h3>
				<span className="text-xs font-semibold text-primary">
					{ sprintf(

						/* translators: 1: current section number, 2: total sections */
						__( 'Section %1$d of %2$d', 'burst-mainwp' ),
						sectionNumber,
	...
	const content = (
		<div className="flex flex-col gap-4">
			<p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">
				{ __( 'You have previous tour progress. Would you like to resume from where you left off or start fresh from the beginning?', 'burst-mainwp' ) }
			</p>
	...
	const footer = (
		<div className="flex flex-col gap-2.5 w-full">
			...
			>
				<span>{ __( 'Start over from beginning 🔄', 'burst-mainwp' ) }</span>
			</button>
		</div>
	);
