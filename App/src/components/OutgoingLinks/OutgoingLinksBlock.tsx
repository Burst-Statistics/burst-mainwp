	...
	if ( ! isLicenseValid ) {
		return (
			<Block className={ `${ className } relative min-h-[320px] overflow-hidden` }>
				<BlockHeading title={ __( 'Outgoing links', 'burst-mainwp' ) } />
				<BlockContent className="px-0 py-0 overflow-y-auto">
					<div className="flex h-48 flex-col items-center justify-center p-4 text-center text-sm text-gray-400 select-none blur-[1px]">
						<p className="font-medium text-gray-500 mb-1">
							{ __( 'Outgoing links tracking is a Pro feature.', 'burst-mainwp' ) }
						</p>
					</div>
					<UpsellOverlay
			...
			<BlockHeading
				...
				title={ <>
					<MetricInfo metricKey="outgoing_links" side="bottom">
						{ __( 'Outgoing links', 'burst-mainwp' ) }
					</MetricInfo>
					{ isEnabled && hasData && (
						<button
							type="button"
			...
			<BlockContent className="px-0 py-0 overflow-y-auto">
				{ ! isEnabled && (
					<div className="flex h-48 flex-col items-center justify-center p-4 text-center text-sm text-gray-500">
						<p className="font-medium text-gray-600 mb-1">
							{ __( 'External link tracking has been disabled.', 'burst-mainwp' ) }
						</p>
						<p className="text-xs text-gray-400">
							{ __( 'You can enable it in the settings page to start tracking outgoing link clicks.', 'burst-mainwp' ) }
						</p>
					</div>
				) }
				{ isEnabled && (
					<BarDataTable
						...
						barColumnKey="clicks"
						isLoading={ isLoading }
						emptyState={ firstCycleCompleted ? __( 'No outgoing link clicks recorded yet.', 'burst-mainwp' ) : '' }
					/>
				) }
				{ isEnabled && ! firstCycleCompleted && (
					<div className="flex items-center gap-2 px-4 py-2 text-xs text-gray-400 border-t border-gray-100">
						<span>
							{ __( 'Burst is gathering all used external links on your site, data will appear when this is completed.', 'burst-mainwp' ) }
