...
export const SearchQueriesInitialPending = ({
	...
	<div className="flex min-h-52 items-center justify-center px-4">
		...
		>
			{ __( 'Fetching Search Console data for this page…', 'burst-mainwp' ) }
		</SearchQueriesProgress>
	</div>
);
...
const getSearchQueriesEmptyMessage = (
	...
	if ( 'failed' === dataStatus ) {
		return __( 'Search Console data could not be fetched. Please try again later.', 'burst-mainwp' );
	}
	if ( 'unavailable' === dataStatus ) {
		if ( isPropertyPaused ) {
			return __( 'Search Console access to this property is paused. Reconnect an account with access to fetch data for this page.', 'burst-mainwp' );
		}
		return __( 'Search Console data is not available for this page yet.', 'burst-mainwp' );
	}
	return __( 'No Search Console queries were found for this page in the selected date range.', 'burst-mainwp' );
};
			...
			<div className="flex min-h-52 items-center justify-center px-4">
				...
				>
					{ sprintf(
						...
						__( 'No queries were found in the checked dates. Checked days: %1$s. Pending days: %2$s.', 'burst-mainwp' ),
