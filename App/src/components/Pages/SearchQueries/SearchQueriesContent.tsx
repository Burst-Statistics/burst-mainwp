	...
	<>
		{isPending && (
			<SearchQueriesProgress
				...
				className="mb-4"
			>
				{ __( 'Showing partial Search Console data while the rest of this date range is fetched in the background.', 'burst-mainwp' ) }
			</SearchQueriesProgress>
		) }
		{isFailed && (
			<p className="mb-4 mt-0 text-sm text-text-gray-light">
				{ __( 'Some Search Console dates could not be fetched. Showing the available data.', 'burst-mainwp' ) }
			</p>
		) }
		{isUnavailable && (
			<p className="mb-4 mt-0 text-sm text-text-gray-light">
				{isPropertyPaused ?
					__( 'Showing stored Search Console data. Reconnect an account with access to fetch missing dates.', 'burst-mainwp' ) :
					__( 'Showing available Search Console data. Ask a site administrator to fetch the missing dates.', 'burst-mainwp' )
				}
			</p>
		) }
