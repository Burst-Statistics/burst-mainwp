...
const SearchQueriesToggle = ({
	...
	return (
		...
		>
			{isExpanded ?
				__( 'Show top queries', 'burst-mainwp' ) :
				sprintf(

					/* translators: %s: number of visible query rows. */
					__( 'All %s queries', 'burst-mainwp' ),
				...
				<table className="w-full min-w-[560px] table-fixed border-separate border-spacing-0 text-sm">
					...
					<thead>
						<tr className="text-left text-text-gray-light">
							<th className="pb-2 font-normal">{ __( 'Query', 'burst-mainwp' ) }</th>
							<th className="pb-2 text-right font-normal">{ __( 'Clicks', 'burst-mainwp' ) }</th>
							<th className="pb-2 text-right font-normal">{ __( 'Impr.', 'burst-mainwp' ) }</th>
							<th className="pb-2 text-right font-normal">{ __( 'Pos.', 'burst-mainwp' ) }</th>
						</tr>
					</thead>
					<tbody>
						<SearchQueriesRows
