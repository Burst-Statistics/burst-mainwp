		...
		>
			...
			{isLoading && (
				<div
					...
					aria-label={__( 'Loading page preview', 'burst-mainwp' )}
				>
			...
			{hasError && (
				<div className="absolute inset-0 flex flex-col justify-between bg-gray-50 p-4">
					...
					<div className="flex items-center justify-between border-t border-gray-200/80 pt-2 text-xs text-text-gray">
						<span>{__( 'Preview unavailable', 'burst-mainwp' )}</span>
					</div>
				</div>
			)}
			...
			{hasPageUrl && ! hasError && (
				<iframe
					...
					title={__( 'Preview of the selected page', 'burst-mainwp' )}
					loading="lazy"
			...
			{( isPreviewLoaded || hasError ) && hasPageUrl && (
				<a
					...
					aria-label={__( 'Visit page', 'burst-mainwp' )}
				>
