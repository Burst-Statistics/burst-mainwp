...
export const PageSummaryHeader = ({
	...
	const readingTime = sprintf(
		_n( '%s min read', '%s min read', data.readingMinutes, 'burst-mainwp' ),
		...
		<div className="flex min-w-0 flex-1 flex-col justify-center">
			<div className="flex items-start justify-between gap-4">
				...
				{editUrl && (
					<ButtonInput
						...
						ariaLabel={__( 'Edit page', 'burst-mainwp' )}
					>
						<Icon name="pencil" size={16} />
						{__( 'Edit page', 'burst-mainwp' )}
					</ButtonInput>
				)}
			</div>
			...
			<div className="mt-4 flex flex-wrap gap-x-5 gap-y-2">
				...
				<MetaItem icon="calendar">
					{sprintf(
						__( 'Published %s', 'burst-mainwp' ),
						formatDate( data.publishedAt )
					)}
				</MetaItem>
				<MetaItem icon="pencil">
					{sprintf(
						__( 'Edited %s', 'burst-mainwp' ),
						relativeEditTime
					)}
				</MetaItem>
				<MetaItem icon="time">
					{sprintf(
						__( '%1$s words · %2$s', 'burst-mainwp' ),
